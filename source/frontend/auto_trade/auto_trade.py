...
from ..common.cur_datetime import cur_datetime
from ..common.df_to_table import df_to_table
...
class AutoTradePage(PageWidget):
    def __init__(self, *args, **kwargs):
        ...
        # thread manager
        self.backend_driver = self.main_window.backend_driver
    ...
    def update_trade_history(self):
        ...
        df = pd.read_csv('backend/trade_history.csv')
    ...
    def set_panel1(self):
        ...
        def invest_clicked():
            ...
            df: pd.DataFrame = pd.read_csv('backend/sessions.csv')
            d = {'date': [cur_datetime()], 'session_num': [algorithm_process.session_id],
                 'algorithm': [self.current_algorithm], 'profit': [0]}    # session data
            df_to_add = pd.DataFrame(d)
            df = df_to_add.append(df, ignore_index=False)
            df.to_csv('backend/sessions.csv', index=False)
            self.update_session()
    ...
    def set_panel3(self):
        ...
        session_df = pd.read_csv("backend/sessions.csv")
        ...
        def kill_session_clicked():
            if self.session_df_row == -1:
                ...
                return
            df = pd.read_csv("backend/sessions.csv")
